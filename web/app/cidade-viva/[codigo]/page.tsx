...
export async function generateMetadata({
  ...
  const data = await buscarCidadeViva(params.codigo);
  if (!data) return { title: "CidadeViva · DadoSabedoria" };
  const local = data.nome + (data.uf ? ` (${data.uf})` : "");
  return {
    title: `CidadeViva — ${local} · DadoSabedoria`,
    description: `Investimento público em urbanismo em ${local}: SICONFI Função 15.`,
  };
}

const ROTULOS_NIVEL: Record<NivelUrbanismo, string> = {
...
const CORES_NIVEL: Record<NivelUrbanismo, string> = {
  expressivo: "#16a34a",
  moderado: "#ca8a04",
  incipiente: "#b45309",
  sem_dado: "#6b7280",
...
export default async function CidadeVivaPage({
  params,
}: {
  params: { codigo: string };
}) {
    ...
    <main className="produto-page">
      ...
      <header className="produto-header">
        ...
        <p className="produto-subtitulo">
          Investimento público municipal em urbanismo — SICONFI Função 15
        </p>
      ...
      <section className="produto-secao" aria-label="Indicadores de investimento">
        ...
        <table
          ...
          aria-label="Investimento municipal em urbanismo"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Despesa liquidada — Função 15 (Urbanismo)</td>
      ...
      <section className="produto-secao" aria-label="Escala de referência">
        <h2>Escala de Referência (Limiares Provisórios — URB-01)</h2>
        <table
          className="produto-tabela"
          aria-label="Escala de classificação do investimento em urbanismo"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Expressivo</td>
              <td>≥ R$ 200</td>
              <td>Município com programa ativo de infraestrutura urbana</td>
            </tr>
            <tr>
              <td>Moderado</td>
              <td>R$ 80 – R$ 199</td>
              <td>Esforço orçamentário dentro do padrão médio</td>
            </tr>
            <tr>
              <td>Incipiente</td>
              <td>&lt; R$ 80 ou zero</td>
              <td>Investimento municipal em urbanismo abaixo do esperado</td>
            </tr>
          </tbody>
        </table>
      </section>
