...
export async function generateMetadata({
  ...
  const data = await buscarEcoVivo(params.codigo);
  if (!data) return { title: "EcoVivo · DadoSabedoria" };
  const local = data.nome + (data.uf ? ` (${data.uf})` : "");
  return {
    title: `EcoVivo — ${local} · DadoSabedoria`,
    description: `Investimento público em gestão ambiental em ${local}: SICONFI Função 18.`,
  };
}

const ROTULOS_NIVEL: Record<NivelAmbiental, string> = {
...
const CORES_NIVEL: Record<NivelAmbiental, string> = {
  expressivo: "#16a34a",
  moderado: "#ca8a04",
  incipiente: "#b45309",
  sem_dado: "#6b7280",
...
export default async function EcoVivoPage({
  params,
}: {
  params: { codigo: string };
}) {
    ...
    <main className="produto-page">
      ...
      <header className="produto-header">
        ...
        <p className="produto-subtitulo">
          Investimento público municipal em gestão ambiental — SICONFI Função 18
        </p>
      ...
      <section className="produto-secao" aria-label="Indicadores de investimento">
        ...
        <table
          ...
          aria-label="Investimento municipal em gestão ambiental"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Despesa liquidada — Função 18 (Gestão Ambiental)</td>
      ...
      <section className="produto-secao" aria-label="Escala de referência">
        <h2>Escala de Referência (Limiares Provisórios — AMB-01)</h2>
        <table
          className="produto-tabela"
          aria-label="Escala de classificação do investimento em gestão ambiental"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Expressivo</td>
              <td>≥ R$ 30</td>
              <td>Município com programa ambiental ativo</td>
            </tr>
            <tr>
              <td>Moderado</td>
              <td>R$ 5 – R$ 29</td>
              <td>Algum esforço orçamentário em gestão ambiental</td>
            </tr>
            <tr>
              <td>Incipiente</td>
              <td>&lt; R$ 5 ou zero</td>
              <td>Investimento mínimo ou ausente na função 18</td>
            </tr>
          </tbody>
        </table>
      </section>
