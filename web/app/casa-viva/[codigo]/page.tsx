...
export async function generateMetadata({
  ...
  const data = await buscarCasaViva(params.codigo);
  if (!data) return { title: "CasaViva · DadoSabedoria" };
  const local = data.nome + (data.uf ? ` (${data.uf})` : "");
  return {
    title: `CasaViva — ${local} · DadoSabedoria`,
    description: `Investimento público em habitação em ${local}: SICONFI Função 16.`,
  };
}

const ROTULOS_NIVEL: Record<NivelHabitacao, string> = {
...
const CORES_NIVEL: Record<NivelHabitacao, string> = {
  expressivo: "#16a34a",
  moderado: "#ca8a04",
  incipiente: "#b45309",
  sem_dado: "#6b7280",
...
export default async function CasaVivaPage({
  params,
}: {
  params: { codigo: string };
}) {
    ...
    <main className="produto-page">
      ...
      <header className="produto-header">
        ...
        <p className="produto-subtitulo">
          Investimento público municipal em habitação — SICONFI Função 16
        </p>
      ...
      <section className="produto-secao" aria-label="Indicadores de investimento">
        ...
        <table
          ...
          aria-label="Investimento municipal em habitação"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Despesa liquidada — Função 16 (Habitação)</td>
      ...
      <section className="produto-secao" aria-label="Escala de referência">
        <h2>Escala de Referência (Limiares Provisórios — HAB-02)</h2>
        <table
          className="produto-tabela"
          aria-label="Escala de classificação do investimento em habitação"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Expressivo</td>
              <td>≥ R$ 50</td>
              <td>Município com programa habitacional ativo</td>
            </tr>
            <tr>
              <td>Moderado</td>
              <td>R$ 10 – R$ 49</td>
              <td>Algum esforço orçamentário em habitação</td>
            </tr>
            <tr>
              <td>Incipiente</td>
              <td>&lt; R$ 10 ou zero</td>
              <td>Investimento mínimo ou ausente na função 16</td>
            </tr>
          </tbody>
        </table>
      </section>
