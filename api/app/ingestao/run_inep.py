"""CLI de backfill/execução: ``python -m app.ingestao.run_inep <ano>`` (Censo Escolar, anual).

Vivo-pronto: a esteira completa roda; a 1ª busca real (host ``download.inep.gov.br`` no allowlist)
confirma a forma. Sem isso, o fetcher real responde 403 (contêiner github-only).
"""
...
async def _main(ano: int) -> None:  # pragma: no cover - rede/S3
    ...
    async with connect(settings.database_url) as conn:
        await executar_inep(Janela(ano, 1), conn, adaptador, store, responsavel="cli")


def main() -> None:  # pragma: no cover - entrypoint
    if len(sys.argv) != 2:
        raise SystemExit("uso: python -m app.ingestao.run_inep <ano>")
