"""CLI de backfill/execução manual: ``python -m app.ingestao.run_siconfi <ano>`` (DCA é anual)."""
...
async def _main(ano: int) -> None:  # pragma: no cover - rede/S3
    ...
    async with connect(settings.database_url) as conn:
        await executar_siconfi(Janela(ano, 1), conn, adaptador, store, responsavel="cli")


def main() -> None:  # pragma: no cover - entrypoint
    if len(sys.argv) != 2:
        raise SystemExit("uso: python -m app.ingestao.run_siconfi <ano>")
