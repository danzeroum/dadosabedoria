...
import { buscarCulturaViva } from "../../../lib/api";
import { ProdutosRelacionados } from "../../../components/ProdutosRelacionados";
import type { NivelCultura } from "../../../lib/types";
...
export async function generateMetadata({
  ...
  const data = await buscarCulturaViva(params.codigo);
  if (!data) return { title: "CulturaViva · DadoSabedoria" };
  const local = data.nome + (data.uf ? ` (${data.uf})` : "");
  return {
    title: `CulturaViva — ${local} · DadoSabedoria`,
    description: `Investimento público em cultura em ${local}: SICONFI Função 13.`,
  };
}

const ROTULOS_NIVEL: Record<NivelCultura, string> = {
...
const CORES_NIVEL: Record<NivelCultura, string> = {
  expressivo: "#16a34a",
  moderado: "#ca8a04",
  incipiente: "#b45309",
  sem_dado: "#6b7280",
...
export default async function CulturaVivaPage({
  params,
}: {
  params: { codigo: string };
}) {
    ...
    <main className="produto-page">
      ...
      <header className="produto-header">
        ...
        <p className="produto-subtitulo">
          Investimento público municipal em cultura — SICONFI Função 13
        </p>
      ...
      <section className="produto-secao" aria-label="Indicadores de investimento">
        ...
        <table
          ...
          aria-label="Investimento municipal em cultura"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Despesa liquidada — Função 13 (Cultura)</td>
      ...
      <section className="produto-secao" aria-label="Escala de referência">
        <h2>Escala de Referência (Limiares Provisórios — CULT-01)</h2>
        <table
          className="produto-tabela"
          aria-label="Escala de classificação do investimento em cultura"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Expressivo</td>
              <td>≥ R$ 30</td>
              <td>Município com equipamentos culturais relevantes (bibliotecas, teatros)</td>
            </tr>
            <tr>
              <td>Moderado</td>
              <td>R$ 10 – R$ 29</td>
              <td>Algum esforço orçamentário direto em cultura</td>
            </tr>
            <tr>
              <td>Incipiente</td>
              <td>&lt; R$ 10 ou zero</td>
              <td>Investimento municipal em cultura abaixo do padrão médio</td>
            </tr>
          </tbody>
        </table>
      </section>
      ...
      <ProdutosRelacionados slug="cultura-viva" codigoIbge={data.codigo_ibge} />
    </main>
  );
}
