...
export async function generateMetadata({
  ...
  const data = await buscarEscolaViva(params.codigo);
  if (!data) return { title: "EscolaViva · DadoSabedoria" };
  const local = data.nome + (data.uf ? ` (${data.uf})` : "");
  return {
    title: `EscolaViva — ${local} · DadoSabedoria`,
    description: `Investimento público em educação em ${local}: SICONFI Função 12.`,
  };
}

const ROTULOS_NIVEL: Record<NivelEducacaoPublica, string> = {
...
const CORES_NIVEL: Record<NivelEducacaoPublica, string> = {
  expressivo: "#16a34a",
  moderado: "#ca8a04",
  incipiente: "#b45309",
  sem_dado: "#6b7280",
...
export default async function EscolaVivaPage({
  params,
}: {
  params: { codigo: string };
}) {
    ...
    <main className="produto-page">
      ...
      <header className="produto-header">
        ...
        <p className="produto-subtitulo">
          Investimento público municipal em educação — SICONFI Função 12
        </p>
      ...
      <section className="produto-secao" aria-label="Indicadores de investimento">
        ...
        <table
          ...
          aria-label="Investimento municipal em educação"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Despesa liquidada — Função 12 (Educação)</td>
      ...
      <section className="produto-secao" aria-label="Escala de referência">
        <h2>Escala de Referência (Limiares Provisórios — EDU-03)</h2>
        <table
          className="produto-tabela"
          aria-label="Escala de classificação do investimento em educação"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Expressivo</td>
              <td>≥ R$ 600</td>
              <td>Município com forte rede educacional própria</td>
            </tr>
            <tr>
              <td>Moderado</td>
              <td>R$ 200 – R$ 599</td>
              <td>Esforço educacional dentro do padrão médio</td>
            </tr>
            <tr>
              <td>Incipiente</td>
              <td>&lt; R$ 200 ou zero</td>
              <td>Investimento abaixo do esperado — verificar repasses FNDE</td>
            </tr>
          </tbody>
        </table>
      </section>
