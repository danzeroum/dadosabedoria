"""CLI de ingestão: ``python -m app.ingestao.run_sinan <ano>`` (SINAN/Dengue, anual).

Vivo-pronto: a esteira completa existe; a 1ª busca real requer
``ftp.datasus.gov.br`` no allowlist do ambiente (VPS).
"""
...
async def _main(ano: int) -> None:  # pragma: no cover - rede
    ...
    async with connect(settings.database_url) as conn:
        await executar_sinan(Janela(ano, 1), conn, adaptador, store, responsavel="cli")


def main() -> None:  # pragma: no cover - entrypoint
    if len(sys.argv) != 2:
        raise SystemExit("uso: python -m app.ingestao.run_sinan <ano>")
