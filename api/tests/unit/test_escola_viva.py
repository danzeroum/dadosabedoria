"""Testes unitários do produto EDU-03 EscolaViva."""

from __future__ import annotations

import pytest

from app.produtos.escola_viva import (
    NOTA_HONESTA,
    calcular,
    classificar_nivel,
)
...
def test_classificar_nivel_expressivo() -> None:
    assert classificar_nivel(600.0) == "expressivo"
    assert classificar_nivel(2000.0) == "expressivo"
...
def test_classificar_nivel_moderado() -> None:
    assert classificar_nivel(200.0) == "moderado"
    assert classificar_nivel(400.0) == "moderado"
    assert classificar_nivel(599.99) == "moderado"


def test_classificar_nivel_incipiente() -> None:
    assert classificar_nivel(0.0) == "incipiente"
    assert classificar_nivel(100.0) == "incipiente"
    assert classificar_nivel(199.99) == "incipiente"
...
def test_calcular_municipio_expressivo() -> None:
    # R$ 60M / 80k hab = R$ 750/hab → expressivo
    ev = calcular(
        "3550308",
        "São Paulo",
        "SP",
        80_000,
        ano=2024,
        valor_liquidado=60_000_000.0,
    )
    assert ev.codigo_ibge == "3550308"
    assert ev.nome == "São Paulo"
    assert ev.uf == "SP"
    assert ev.populacao == 80_000
    assert ev.ano == 2024
    assert ev.valor_liquidado == pytest.approx(60_000_000.0)
    assert ev.valor_por_hab == pytest.approx(750.0)
    assert ev.nivel == "expressivo"


def test_calcular_municipio_moderado() -> None:
    # R$ 12M / 40k hab = R$ 300/hab → moderado
    ev = calcular(
        "5000002",
        "Municipio Moderado",
        "MS",
    ...
    assert ev.valor_por_hab == pytest.approx(300.0)


def test_calcular_municipio_incipiente() -> None:
    # R$ 5M / 50k hab = R$ 100/hab → incipiente
    ev = calcular(
        "5000001",
    ...
    assert ev.valor_por_hab == pytest.approx(100.0)
...
def test_calcular_sem_populacao_retorna_none_por_hab() -> None:
    ev = calcular("3550308", "SP", "SP", None, ano=2024, valor_liquidado=10_000_000.0)
    assert ev.valor_por_hab is None
    assert ev.nivel == "sem_dado"


def test_calcular_populacao_zero_retorna_none_por_hab() -> None:
    ev = calcular("3550308", "SP", "SP", 0, ano=2024, valor_liquidado=10_000_000.0)
    assert ev.valor_por_hab is None
    assert ev.nivel == "sem_dado"
...
def test_nota_honesta_presente() -> None:
    assert "função 12" in NOTA_HONESTA
    assert "Educação" in NOTA_HONESTA
    assert "FNDE" in NOTA_HONESTA
