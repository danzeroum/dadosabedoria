...
export async function generateMetadata({
  ...
  const data = await buscarSegurancaViva(params.codigo);
  if (!data) return { title: "SegurançaViva · DadoSabedoria" };
  const local = data.nome + (data.uf ? ` (${data.uf})` : "");
  return {
    title: `SegurançaViva — ${local} · DadoSabedoria`,
    description: `Investimento público em segurança pública em ${local}: SICONFI Função 06.`,
  };
}

const ROTULOS_NIVEL: Record<NivelSeguranca, string> = {
...
const CORES_NIVEL: Record<NivelSeguranca, string> = {
  expressivo: "#16a34a",
  moderado: "#ca8a04",
  incipiente: "#b45309",
  sem_dado: "#6b7280",
...
export default async function SegurancaVivaPage({
  params,
}: {
  params: { codigo: string };
}) {
    ...
    <main className="produto-page">
      ...
      <header className="produto-header">
        ...
        <p className="produto-subtitulo">
          Investimento público municipal em segurança pública — SICONFI Função 06
        </p>
      ...
      <section className="produto-secao" aria-label="Indicadores de investimento">
        ...
        <table
          ...
          aria-label="Investimento municipal em segurança pública"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Despesa liquidada — Função 06 (Segurança Pública)</td>
      ...
      <section className="produto-secao" aria-label="Escala de referência">
        <h2>Escala de Referência (Limiares Provisórios — SEG-01)</h2>
        <table
          className="produto-tabela"
          aria-label="Escala de classificação do investimento em segurança pública"
        >
          <thead>
            <tr>
          ...
          <tbody>
            <tr>
              <td>Expressivo</td>
              <td>≥ R$ 100</td>
              <td>Município com GCM estruturada e defesa civil ativa</td>
            </tr>
            <tr>
              <td>Moderado</td>
              <td>R$ 30 – R$ 99</td>
              <td>Algum esforço orçamentário direto em segurança pública municipal</td>
            </tr>
            <tr>
              <td>Incipiente</td>
              <td>&lt; R$ 30 ou zero</td>
              <td>Sem GCM própria — segurança custeada principalmente pelo estado</td>
            </tr>
          </tbody>
        </table>
      </section>
