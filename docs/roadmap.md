...
> **SESSÃO 2026-06-12 (PRs #129–#137):** completude de produtos + cobertura + rate-limit + quota + lote paralelo + gateway.
...
> - **PR #136 — Consultas-lote paralela:** `asyncio.gather` + `asyncio.Semaphore(5)`, sessão própria por item; `CONCORRENCIA_LOTE` configurável.
> - **PR #137 — Rate-limit Traefik tier profundo:** `dsab-ratelimit-profundo` (10 req/s, burst 5); router `api-profundo` com prioridade sobre `/v1` geral.
...
- [~] **Allowlist dos conectores vivos (lote, a adicionar em bloco no #0):** os conectores foram
  ...
  **#0 sondado 2× (2026-06-07 + 2026-06-08, sessão nova a cada vez; ADR-0028 + ADR-0033):**
  ...
  - ⚠️ **CAGED** `ftp.mtps.gov.br` — FTP não acessível neste contêiner (porta 21 bloqueada). **Fase 1
    completa (PR #111, ADR-0036):** fixture fiel-à-forma, fetcher passivo, streaming nacional, runbook.
    Falta apenas liberar `ftp.mtps.gov.br:21` no allowlist da **VPS** (não deste contêiner) e rodar
    `python -m app.ingestao.run_caged 2026 4` em tmux (ver `RUNBOOK_DEPLOY.md §CAGED go-live`).
...
- [x] 🟢 **Camada profunda paga:** `POST /v1/consultas-lote` (lote sobre o acervo público — paga-se
  ...
  *(Rate-limit no gateway Traefik + consulta-lote otimizada: ambos entregues — PR #136 + PR #137.)*
...
- [x] 🔵 **SOCIAL-01 AssisViva** (SICONFI Função 08) — endpoint `GET /v1/assis-viva/{ibge}` + tela `/assis-viva/{codigo}`; despesa liquidada na função 08 (Assistência Social) por habitante; níveis expressivo≥R$150/hab, moderado≥R$50, incipiente<R$50; nota honesta (Bolsa Família/BPC não incluso; municípios-polo têm demanda regional; dupla face §17); 12 testes unitários + 4 integração; 2026-06-13.
- [x] 🔵 **CULT-01 CulturaViva** (SICONFI Função 13) — endpoint `GET /v1/cultura-viva/{ibge}` + tela `/cultura-viva/{codigo}`; despesa liquidada na função 13 (Cultura) por habitante; níveis expressivo≥R$30/hab, moderado≥R$10, incipiente<R$10; nota honesta (Lei Rouanet/fundos estaduais não inclusos; municípios turísticos tendem a investir mais; dupla face §17); 12 testes unitários + 4 integração; 2026-06-13.
- [x] 🔵 **SEG-01 SegurançaViva** (SICONFI Função 06) — endpoint `GET /v1/seguranca-viva/{ibge}` + tela `/seguranca-viva/{codigo}`; despesa liquidada na função 06 (Segurança Pública) por habitante; níveis expressivo≥R$100/hab, moderado≥R$30, incipiente<R$30; nota honesta (PM/PC estaduais não inclusos; municípios sem GCM têm gasto próximo de zero; dupla face §17); 12 testes unitários + 4 integração; 2026-06-13.
